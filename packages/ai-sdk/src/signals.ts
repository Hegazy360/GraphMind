 ...
 * neutralized: `ai`'s `timeout` configs (`totalMs`/`stepMs`/`toolMs`/...) all
 * materialize as `AbortSignal.timeout()`-style aborts whose reason is named
 * `TimeoutError`, and a held gate would otherwise burn that budget and kill
 * the run mid-debug. Aborts with any other reason (user aborts) pass through
 * untouched. When detached, the adapter does not touch signals at all.
 */

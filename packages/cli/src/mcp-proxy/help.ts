...
export function mcpProxyHelp(port: number = DEFAULT_PORT): string[] {
  return [
    ...
    'a JSON-RPC error at the error breakpoint `graphmind serve` arms (turn it',
    'off with `serve --pause-on-error off`), an isError result by a hold in',
    'the proxy itself (turn it off with GRAPHMIND_BREAK_ON_ERROR_RESULT=0).',
    '',
    'Options:',
    ...
    '  GRAPHMIND_BREAK_ON_ERROR_RESULT=0  do not hold isError results (JSON-RPC',
    '                                 errors still follow the error breakpoint)',
    '  GRAPHMIND_DISABLE_EDIT_INPUT=1 refuse every edit of a held call\'s arguments',

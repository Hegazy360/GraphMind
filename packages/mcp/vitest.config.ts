...
export default defineConfig({
  ...
  test: {
    // Several suites assert real elapsed time (gate holds, detached overhead,
    // disconnect auto-continue). Hosted CI runners are noisy neighbours, so
    // give those two retries there; locally a failure is a failure, first time.

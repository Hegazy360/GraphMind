...
callback-only gate — `before`, `after` or `error` — the app refuses it (`exec.refused`, code
`unsupported`) and the gate stays held for `continue` or `abort`. It never pretends to have
substituted or re-run something, which is why the answer is always to wrap the tool rather than
to keep clicking `Inject` on the node.
...
sanitiser: cycles become `'[Circular]'` and unserialisable values degrade instead of throwing.
Since 0.6 nothing is cut to a preview by default — prompts and states are recorded in full, and
the 512 KB per-event budget every GraphMind client applies is the only bound. Set
`maxPayloadChars` to get the old behaviour (anything over it is replaced by a truncated preview
marker).

LLM steps also record, since 0.6, the sampling parameters from LangChain's `invocation_params`
and the bound tools as `{name, schemaHash}` (each definition once per run as `toolSchemas`,
without credential-like keys outside its schema) on
`node.started`, and on `node.finished` the requested `toolCalls` (LangChain's
`invalid_tool_calls` keep their raw text as `inputText`), the normalized `finishReason` with the
provider's own `rawFinishReason`, and **inclusive** usage — `usage_metadata` first (LangChain
already counts cached tokens in `input_tokens`), else the raw provider usage, where Anthropic's
uncached `input_tokens` gets its cache reads and writes added back (see [usage and
cost](/debugging/inspecting/#usage-and-cost)).

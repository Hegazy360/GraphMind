...
Peer ranges: `@langchain/core >=0.3 <2` (validated against 1.2.9) and an optional
`@langchain/langgraph >=0.2 <2` (validated against 1.4.13).
...
callback-only gate, the adapter warns once — naming the wrapper you need — and continues. It
never silently pretends to have substituted something.

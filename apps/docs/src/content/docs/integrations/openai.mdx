...
Normalised onto the wire's **inclusive** usage (see [usage and
cost](/debugging/inspecting/#usage-and-cost)): `inputTokens` is `prompt_tokens` /
`input_tokens`, which already include cached tokens, with `inclusive: true`;
`cacheReadTokens` (`cached_tokens`), `cacheWriteTokens` (`cache_write_tokens`) and
`reasoningTokens` appear when the response reports them. `totalTokens` and `cachedInputTokens`
(the 0.5 name of `cacheReadTokens`) are kept as loose fields through 0.6.x.

`node.finished` also carries the requested `toolCalls: [{id, name, input}]` (arguments that do
not parse — cut off by the token limit — are kept as `inputText`) and the normalized
`finishReason` (`stop`, `length`, `tool-calls`, `content-filter`, …) with the API's own value
as `rawFinishReason` (Responses: the incomplete reason, else the status). `node.started`
records the request as sent: every message / input in full, the sampling parameters under the
API's names (`temperature`, `max_completion_tokens`, `top_p`, `stop`, `seed`, `tool_choice`,
`reasoning`, …; `metadata`, `user` and request options are never read) and `tools` as `{name,
schemaHash}` with each definition sent once per run as `toolSchemas` — without credential-like
keys outside its schema (an `mcp` tool's headers and authorization, a URL's user:password and
query), and again intact on the next step if the 512 KB event limit shrank it.
...
- Disabled session: `wrapClient` / `wrapTools` return their inputs unchanged (identity).

...
sequence numbers when relaying to apps. The server refuses an `exec.resume` with action
`inject` whose output still contains `"__REDACTED__"`. It answers `{ type: 'error', runId,
message: 'inject refused: …' }` and does not relay it.
